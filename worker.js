  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/claude') {
      ...
      try {
        ...
        const ALLOWED_MODELS = ['claude-sonnet-4-20250514', 'claude-haiku-4-5-20251001', 'claude-opus-4-20250514', 'claude-opus-4-6-20250610'];
    ...
    if (url.pathname === '/api/gkp-forecast' && request.method === 'POST') {
      ...
      try {
        ...
        const forecastRes = await fetch(
          'https://googleads.googleapis.com/v23/customers/' + customerId + ':generateKeywordForecastMetrics',
          {
            method: 'POST',
            headers: gadsHeaders(env, token),
            body: JSON.stringify({
