...
1. **Setup** — Client name, URL, primary market, metro/city targeting (multi-select for GKP geo data), industry, uploaded strategy docs (auto-extract to client goals), discovery notes, **Core Focus** (must-win terms — products/tools/platforms that anchor the entire keyword strategy, stored in `S.setup.coreFocus[]`, feeds into mechanical seeds, AI seeds, and AI-Select with force-include guarantee), client goals (goal statement, measurable target, baseline, timeline, primary KPI), **Strategic Directives** (global rules: Never/Always/Prefer types, injected into every AI prompt across all stages via `getDirectivesContext()`), sales qualification, competitor URLs, Analytics Connection card (GSC property picker + GA4 property picker, auto-saves on change), Webflow Connection card (API token → site selector → stores in KV per project, enables form submission pull + future schema/alt text/layout push), GoHighLevel Connection card (API token + location ID → stores in KV per project, enables contact pull into Leads), call transcripts (multiple, with auto-extraction of pain points, objections, quotes, emotions, criteria, situation)
2. **Snapshot** — Two top-level tabs: **Site Audit** and **Leads**. Site Audit: Automated domain authority pull (exact Ahrefs DR via `/api/ahrefs-dr` when `AHREFS_API_KEY` set, falls back to DataForSEO approximation), backlink count, top organic pages (collapsible table), competitor DA comparison (auto-pulls competitor DR for all Research competitors), Sales Summary card, Brand SERP card (Knowledge Panel presence, owns #1 result, SERP features, People Also Search For, ratings — via `/api/snapshot-brand-serp`), Current Site Architecture (3 tabs: Insights — AI-generated plain-English description of information architecture/URL patterns/hierarchy depth, Tree — indented hierarchy with traffic badges, Diagram — Mermaid flowchart with grouped view for 60+ page sitemaps), Core Web Vitals, Tech Stack (HTML-based fallback detecting 70+ technologies across 15 categories when DataForSEO returns empty), Schema detection, redirect map (collapsible table), GSC search analytics pull (queries + pages, 90-day window), GA4 metrics pull (sessions, bounce rate, conversions by page + channel groupings). Clear All button to reset snapshot data. **Leads tab**: Lead tracking table with manual add, inline status dropdowns (new/contacted/qualified/proposal/closed_won/closed_lost/disqualified), month filter, value tracking, source badges (Webflow/GHL/Shopify/Manual). Pull from Webflow form submissions via Data API. Webhook receiver for GHL/Shopify/other sources (`/api/webhook/lead/:projectId`). Data stored in `S.snapshot.leads[]`.
3. **Research** — AI-enriched factual data collection across 5 tabs (Business, Audience, Brand, Schema/Local, Competitors) with completeness scorecard, field-level source badges, unit economics inputs. Brand tab now owns all social proof: testimonials, reviews, case studies (with URL field), notable clients — moved from Schema/Local and unified under "Social Proof" section. "Enrich New Competitors" button auto-fills URL/strengths/weaknesses for manually-added competitor names. Competitor discovery uses 3-layer filtering: 70+ domain blocklist, business model matching in AI prompt, post-AI programmatic filter. Competitor type column (direct/indirect). Web scraper extracts comprehensive JSON-LD schema data (LocalBusiness, Product, BreadcrumbList, Event, VideoObject) and auto-populates research fields. Buyer psychology section (JTBD Force Map: push/pull/anxieties/habits with quotes + intensity — auto-populated during enrichAll after client-pain tab, buyer sophistication 1-5, perceived categories, switching triggers, decision criteria with must-have/nice-to-have priority, coverage map with AI Generate Missing button), read-only Current Performance card at top of Business tab (GSC + GA4 data from Snapshot: Search Console, Analytics Overview, Traffic Channels, Top Converting Pages), source attribution banners per tab (Website Scrape, GMB, Documents, Discovery Notes, Brand Assets, Structured Scrape, GSC, GA4 with green/amber badges and refresh buttons). AI enrichment prompt explicitly clarifies extraction is about the CLIENT's services, not what Setsail would sell.
4. **Strategy** — The strategy engine: 10 AI diagnostics (D0–D9) with **7-layer pipeline** (D0 → KW Data → D1-D3 → D4+Select → D5+Cluster → D6 → D7-D9), scoring engine with anti-inflation caps, positioning direction system, budget-tier channel allocation, audience intelligence, sensitivity analysis, demand validation, narrative & messaging synthesis (StoryBrand, messaging pillars, objection maps, content hooks, VoC swipe file), interactive Gantt timeline with cost labels, compiled 13-section strategy document with 6 data appendices + revenue projection model, Pricing Engine integration (live service costs, package tier matching, investment summary, internal margin analysis), Service Scope panel (product selection with Low/Mid/High scope, per-service ROI, dual suggested/realistic budget view, scope notes), industry benchmarks (30 verticals with sourced CVR/CPL/close rate/retention data, benchmark table viewer modal with CSV download in Economics tab). 10 tabs: Audience, Positioning, Economics, Subtraction, Channels (merged with Growth Plan — includes Gantt, budget allocation, scope panel), Website, Content & Authority, Risks, Narrative, Output. Keywords panel is persistent above tabs (collapsible). "From here forward" button re-runs current diagnostic through D9. Three distinct re-run buttons: Regenerate All (full), Re-run All (no enrichment, layered: D0 → KW Phase 1 → D1-D3 → D4 + KW Phase 2 → D5 + KW Phase 3 → D6-D9), Improve Weakest (surgical). Auto-compiles strategy doc + web brief after full runs. Strategy document download includes website strategy brief. Source attribution banners per tab (Research %, Snapshot DR, Competitors, Industry Benchmarks, Pricing Catalog, GKP Data, VoC, Tech Stack, diagnostic completion with refresh buttons). GSC/GA4 data flows into D1/D5/D6/D8 via `_snapshotCtxBlock`.
5. **Sitemap** — Two top-level tabs: **Site Pages** (1200px wide) and **Blog Pipeline**. Site Pages: page architecture with keyword-to-page mapping. 5-step workflow strip (Import → Generate → Align → Enrich → Approve) with status dashboard. Issues always visible when they exist (not behind a click), with "Fix All" button that chains no-kw → zero-vol → cannibalisation fixes. "Ready to approve" banner when clean + enriched. Contextual toolbar with Tools dropdown (Goals, Pillars, Personas, Re-suggest, Architecture, Structure Review). Smart cannibalisation detection: same page type = error (fixable), different page types = info (different intent), location pages with geo keywords = info (correct local SEO). Blog Pipeline: card-based view of blog posts with approve/edit/remove actions, stats bar (post count, total volume, needs review, approved, briefed), Blog Discovery card (PAA questions + competitor gap keywords, ranked opportunities table, queued topics), Add Post button, Approve All batch action. Strategy-aware build: D5 page injection (vertical/location/content pages), CTA landing page stubs, pages-to-cut annotation, tier page count enforcement, engagement scope awareness. GSC traffic badges on pages, cannibalisation detection from overlapping GSC queries. Page triage system (Removed tab) flags parameter pages, utility bloat, team sub-pages, cannibalised keywords, off-strategy pages. Realign button for post-strategy updates without full rebuild. Issues panel with AI-powered Fix buttons (keyword assignment, zero-vol replacement, cannibalisation resolution). Full Build button chains all steps: build → priorities → personas → AI keyword fix → page goals (batched, 12/call) → content pillars → live volumes. Architecture diagram with grouped view for 60+ pages. Source badges (D5/CTA/CUT). Strategy alignment columns, content pillar tags, persona/voice/awareness assignment, positioning direction gap detection, CTA gap detection, persona coverage check, budget-tier priority suggestions, market overrides, active page import from Snapshot, Clear All button
...
10. **Schema** — Deterministic JSON-LD assembly from all previous stages (no AI calls). 30+ page types supported: homepage (Organization, WebSite, SearchAction, FAQPage, HowTo, OfferCatalog, Reviews), service (ProfessionalService, FAQPage, HowTo, VideoObject, Reviews), location (LocalBusiness, FAQPage), blog (BlogPosting, FAQPage), product (Product, Offer, Reviews), plus recipe, event, course, careers, news, guide, gallery, software, comparison, podcast, webinar, glossary, and more. "Generate All Schemas" button — instant, no queue. Per-page rich results preview badges. Google Rich Results Test + Schema.org validator integration. Validation with errors/warnings per Google requirements
...
```
worker.js        — All backend API routes + queue consumer + webhooks (~4748 lines)
...
schema.js        — Stage 10: deterministic JSON-LD assembly, 30+ page types (~1434 lines)
prompts.js       — Shared AI prompt templates (~174 lines)
export.js        — Stage 11: export packaging + strategy/investment tabs (~147 lines)
setsailai.js     — SetsailAI assistant: sidepanel with ask/audit/explain/action modes (~1592 lines)
...
- `S.setup` — client info, docs, voice, competitors, sales qualification, client goals (`goalStatement`, `goalTarget`, `goalBaseline`, `goalTimeline`, `goalKpi`), `coreFocus[]` (must-win terms — products/tools/platforms, feeds all keyword stages), `directives[]` (strategic directives: `{type, text, addedAt}`, type: never/always/prefer), metro targeting (`geoMetro`, comma-separated city IDs for multi-city), `gscProperty` (selected GSC property URL), `ga4Property` (selected GA4 property ID), `transcripts[]` (label, text, addedAt, charCount, extracted: pain_points, objections, quotes, emotions, criteria, situation)
- `S.snapshot` — DataForSEO domain metrics, `topPages`, `techStack`, `vitals`, `schemas`, `_insights` (cached AI summary), `gsc` (`queries`, `pages`, `_pulledAt`), `ga4` (`pageMetrics`, `totals`, `channels`, `_pulledAt`), `leads[]` (each: `id`, `contact_name`, `contact_email`, `source_type`, `source_channel`, `source_detail`, `source_page`, `deal_value`, `status`, `lead_date`, `created_at`)
...
- **Webflow:** `/api/webflow/save-token` (store API token per project), `/api/webflow/sites` (list sites), `/api/webflow/forms` (list forms for site), `/api/webflow/form-submissions` (pull submissions → leads), `/api/webflow/disconnect`
- **GoHighLevel:** `/api/ghl/save-token` (store API token + location ID per project), `/api/ghl/contacts` (pull contacts → leads), `/api/ghl/disconnect`
- **Lead webhooks:** `/api/webhook/lead/:projectId?token=xxx` (receive leads from any source), `/api/webhook/token/:projectId` (generate webhook token)
- **Changelog:** `/api/changelog` (GET list, POST add/react/delete — global, stored in `global:changelog` KV key)
...
- **⟳ Keyword Pipeline** (split into 3 phases across layers): Phase 1 (Layer 1, after D0): seeds → DataForSEO expansion → volumes → GKP enrichment. Phase 2 (Layer 3, after D4): AI-Select with full D0-D4 context. Phase 3 (Layer 4, after D5): clustering with D5 page type guidance. D1 Economics now gets real CPC data on first pass (no re-run needed).
...
The keyword pipeline has 10 steps split into 3 phases for the layered architecture:
- **Phase 1 (`kwPhase1_DataCollection`)**: Generate Questions → AI Generate Seeds → Build Mechanical Seeds → Pull Competitor Keywords → Merge All Sources → Fetch Volumes → GKP Enrich. Runs at Layer 1 (after D0, before D1). Core Focus terms from Setup drive seed generation — each gets 50+ mechanical variants. Pinned keywords expanded with geo/modifier/question forms. Product/tool specializations extracted from goals, positioning, service detail, and differentiators.
- **Phase 2 (`kwPhase2_Selection`)**: AI-Select with full D0-D4 context. Runs at Layer 3 (after D4 Channels). Sees channel lever scores for informed selection. Core Focus terms force-included programmatically (every keyword containing a focus term auto-selected as must-have). Pinned keywords guaranteed to survive.
- **Phase 3 (`kwPhase3_Clustering`)**: Cluster into Pages + Audit. Runs at Layer 4 (after D5 Website). Respects D5 page type recommendations.
`runFullKeywordPipeline()` preserved as backward-compat wrapper calling all 3 phases. GKP data now persisted on save (low_bid, high_bid, ad_competition, gkp_volume). Keyword table header is clickable to filter showing only selected keywords.
...
Five additive data signals that feed into brief and copy prompts:
...
- **Roles:** 5-tier system: `super_admin` (ADMIN_EMAIL, untouchable), `admin` (user management), `strategist` (full pipeline), `seo_staff` (execution-focused: hidden sections include Leads tab, client goals, transcripts, proof bank, strategy economics/channels/output tabs, Webflow/GHL tokens), `viewer` (read-only). Auto-registration: new SSO users get `status: 'pending'`, `role: 'viewer'` until admin approves. `ROLE_VISIBILITY` config in index.html controls per-role UI hiding. "View as" dropdown in navbar (admin+ only) for role impersonation testing.
...
### Project guard for async operations
All async operations that modify `S` must check if the project has changed before writing results. Use `projectGuard()`:
```js
var guard = projectGuard();
await someAsyncWork();
if (guard.changed()) { console.warn('project changed, aborting'); return; }
S.pages = result; // safe — same project
```
`resumeProject()` sets `_aiStopAll = true` and clears the save timer to prevent cross-project data contamination. It resets `_aiStopAll = false` after loading the new project. Applied to: `fullBuildSitemap`, `runSnapshot`, `_executePageBrief`.
...
| `global:changelog` | Change Log entries (newest first, max 200). Each: `{title, description, type, author, date, confirmed[], flagged[]}` |
| `webhook_token:{projectId}` | Webhook auth token for lead receiver endpoints |
